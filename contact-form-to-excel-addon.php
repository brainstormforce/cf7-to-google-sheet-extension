...
function save_cf7_gs_settings( $post ) {
	update_post_meta( $post->id(), 'cf7_to_spreadsheet_save', $_POST['cf7-sheet'] );
}
/**
...
function cf7_editor_panel_google_sheet( $post ) { 
	...
	$form_data = get_post_meta( $form_id, 'cf7_to_spreadsheet_save' ); ?>
	...
	<div class="wrap">
		...
		<form  action="options.php" method="post" class="container">
		    ...
		    if ( isset ( $_GET['settings-updated'] ) && get_option( 'cf7_to_spreadsheet_google_code' ) != '' ){
				...
				googlesheet::preauth( get_option('cf7_to_spreadsheet_google_code') ); 
...
function cf7_to_spreadsheet( $cfdata ) {
